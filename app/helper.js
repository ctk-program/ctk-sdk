...
var currentOrgId = hfc.getConfigSetting("curOrgId"); // org id
...
for (const chanindex in chanList) {
	...
	// client channel
	if (isManage) {
		// chan channel id channelTxNum=0
...
function newRemotes(names, forPeers, userOrg) {
	...
	if (targets.length === 0) {
		logger.warn(util.format('Failed to find peers matching the names %s', names));
	}
...
var getChannelForOrg = function (channelName , org) {
	let chans = channels[org];
		...
		console.log(channelName,"=====",chan._name);
	...
	return hfc.newDefaultKeyValueStore({
		...
		return client.getUserContext(username, true).then((user) => {
			if (user && user.isEnrolled()) {
                ...
                console.log(username,"____________________________",password);
